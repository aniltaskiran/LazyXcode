...
class SortSelectedLinesByAlphabeticallyReversed: NSObject, XCSourceEditorCommand {
    func perform(with invocation: XCSourceEditorCommandInvocation, completionHandler: @escaping (Error?) -> Void) {
        SortSelectedRange().sort(buffer: invocation.buffer, by: .alphabeticallyReversed)
        completionHandler(nil)
    }
}
